...
def mini_imagenet_folders():
    train_folder = '../HCFSL-NAS/datasets/miniImagenet/train'
    test_folder = '../HCFSL-NAS/datasets/miniImagenet/test'
    ...
    print('mini-imagenet: ',len(metatrain_folders))
    print('mini-imagenet: ',np.array(metatrain_folders).shape)
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train',shuffle = False):
    ...
    loader = DataLoader(dataset, batch_size=num_per_class*task.num_classes, sampler=sampler) # sampler，shuffle=false
    # loader = DataLoader(dataset, batch_size=32, sampler=sampler) # sampler，shuffle=false
    return loader
