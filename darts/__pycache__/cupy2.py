    ...
    def __init__(self,
        ...
        if compress_stride > 1:
            self.avgpool = nn.Sequential(
                nn.ReflectionPad2d(compress_stride // 2),
                nn.AvgPool2d(compress_stride, 1)
            )
            
            self.maxpool = nn.Sequential(
                nn.ReflectionPad2d(compress_stride // 2),
                nn.MaxPool2d(compress_stride, 1)
            )
...
class Residual(nn.Module):  # pytorch
    def __init__(self,
                 in_channels, out_channels, kernel_size, use_1x1conv=False, stride=1):
        super(Residual, self).__init__()
        self.conv1 = nn.Sequential(
            involution(in_channels, kernel_size, stride=stride),
        ...
        #         if use_1x1conv:
        #             self.conv3 = nn.Sequential(nn.Conv2d(out_channels, in_channels, kernel_size=1, stride=stride),
        #                                        nn.BatchNorm2d(in_channels),
        #                                        nn.ReLU(inplace=True)
        #                                        )
        #         else:
